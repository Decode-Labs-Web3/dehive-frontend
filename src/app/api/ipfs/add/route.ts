...
export async function POST(req: Request) {
  try {
    ...
    if (!postEndpoint) {
      return NextResponse.json({ error: "IPFS_GATEWAY_URL_POST not configured" }, { status: 500 });
    }
    ...
    formData.append("file", new Blob([jsonString], { type: "application/json" }), "payment.json");
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: `IPFS add failed: ${txt}` }, { status: 502 });
    }
    ...
    if (!cid) {
      return NextResponse.json({ error: "CID not found in IPFS response" }, { status: 500 });
    }
